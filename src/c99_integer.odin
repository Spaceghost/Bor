...
write_unsigned_operand :: proc(b: ^strings.Builder, m: ^MIR_Module, id: Value_ID, t: MIR_Type) {
	...
	ct := t == .UIntptr ? "uintptr_t" : "uint32_t"
	strings.write_string(b, "((")
	strings.write_string(b, ct)
	strings.write_string(b, ")(")
	write_value_ref(b, m, id)
	strings.write_string(b, "))")
}

emit_checked_shift :: proc(b: ^strings.Builder, m: ^MIR_Module, op: ^MIR_Inst) {
