package main

import "core:fmt"
import "core:odin/ast"
import "core:odin/tokenizer"
import "core:slice"
import "core:strings"

Lowerer :: struct {
	m:             ^MIR_Module,
	proc_ids:      map[string]Proc_ID,
	global_ids:    map[string]Value_ID,
	locals:        map[string]Value_ID,
	current_proc:  Proc_ID,
	current_block: Block_ID,
	failed:        bool,
}

lower_fail :: proc(l: ^Lowerer, node: ^ast.Node, msg: string, args: ..any) -> bool {
	if node != nil {
		fmt.eprintf("bor: mir: %s:%d:%d: ", node.pos.file, node.pos.line, node.pos.column)
	} else {
		fmt.eprintf("bor: mir: ")
	}
	fmt.eprintf(msg, ..args)
	fmt.eprintf("\n")
	l.failed = true
	return false
}

mir_ident_name :: proc(expr: ^ast.Expr) -> (string, bool) {
	if expr == nil do return "", false
	if n, ok := expr.derived.(^ast.Ident); ok do return n.name, true
	return "", false
}

mir_type_from_ast :: proc(expr: ^ast.Expr) -> MIR_Type {
	if expr == nil do return .Invalid
	#partial switch n in expr.derived {
	case ^ast.Ident:
		switch n.name {
		case "bool":    return .Bool
		case "u8":      return .U8
		case "u32":     return .U32
		case "uintptr": return .UIntptr
		}
	case ^ast.Paren_Expr:
		return mir_type_from_ast(n.expr)
	case ^ast.Multi_Pointer_Type, ^ast.Pointer_Type:
		if mir_type_from_ast(n.elem) == .U8 do return .U8_Ptr
	}
	return .Invalid
}

mir_proc_result :: proc(p: ^ast.Proc_Lit) -> MIR_Type {
	if p == nil || p.type == nil || p.type.results == nil || len(p.type.results.list) == 0 {
		return .Void
	}
	if len(p.type.results.list) != 1 do return .Invalid
	return mir_type_from_ast(p.type.results.list[0].type)
}

mir_calling_convention :: proc(p: ^ast.Proc_Lit) -> string {
	if p == nil || p.type == nil do return ""
	if cc, ok := p.type.calling_convention.(string); ok do return cc
	return ""
}

mir_proc_external :: proc(p: ^ast.Proc_Lit) -> bool {
	cc := mir_calling_convention(p)
	return cc == "c" || cc == "cdecl"
}

new_value :: proc(l: ^Lowerer, kind: Value_Kind, t: MIR_Type, name := "", literal := "") -> Value_ID {
	id := Value_ID(len(l.m.values))
	append(&l.m.values, MIR_Value{kind = kind, type = t, name = name, literal = literal})
	return id
}

new_temp :: proc(l: ^Lowerer, t: MIR_Type) -> Value_ID {
	return new_value(l, .Temp, t)
}

new_literal :: proc(l: ^Lowerer, text: string, t := MIR_Type.UIntptr) -> Value_ID {
	return new_value(l, .Literal, t, literal = text)
}

new_block :: proc(l: ^Lowerer) -> Block_ID {
	id := Block_ID(len(l.m.blocks))
	append(&l.m.blocks, Basic_Block{id = id, first_op = INVALID_INDEX})
	return id
}

emit_op :: proc(l: ^Lowerer, op: MIR_Inst) {
	assert(l.current_block != INVALID_BLOCK)
	append(&l.m.ops, op)
	b := &l.m.blocks[int(l.current_block)]
	b.op_count += 1
}

begin_block :: proc(l: ^Lowerer, id: Block_ID) {
	assert(id != INVALID_BLOCK)
	l.current_block = id
	b := &l.m.blocks[int(id)]
	assert(!b.started)
	b.started = true
	b.first_op = u32(len(l.m.ops))
	emit_op(l, MIR_Inst{kind = .Label, target = id, dst = INVALID_VALUE, a = INVALID_VALUE, b = INVALID_VALUE, callee = INVALID_PROC})
}

block_terminated :: proc(l: ^Lowerer) -> bool {
	if l.current_block == INVALID_BLOCK do return true
	return l.m.blocks[int(l.current_block)].terminated
}

emit_jump :: proc(l: ^Lowerer, target: Block_ID) {
	emit_op(l, MIR_Inst{kind = .Jump, target = target, dst = INVALID_VALUE, a = INVALID_VALUE, b = INVALID_VALUE, callee = INVALID_PROC})
	l.m.blocks[int(l.current_block)].terminated = true
	l.m.blocks[int(target)].incoming += 1
}

emit_jump_if_false :: proc(l: ^Lowerer, cond: Value_ID, target: Block_ID) {
	emit_op(l, MIR_Inst{kind = .Jump_If_False, a = cond, target = target, dst = INVALID_VALUE, b = INVALID_VALUE, callee = INVALID_PROC})
	l.m.blocks[int(l.current_block)].terminated = true
	l.m.blocks[int(target)].incoming += 1
}

lookup_value :: proc(l: ^Lowerer, name: string) -> (Value_ID, bool) {
	if id, ok := l.locals[name]; ok do return id, true
	if id, ok := l.global_ids[name]; ok do return id, true
	return INVALID_VALUE, false
}

value_type :: proc(l: ^Lowerer, id: Value_ID) -> MIR_Type {
	if id == INVALID_VALUE do return .Invalid
	return l.m.values[int(id)].type
}

binary_kind :: proc(text: string) -> (Binary_Op, bool) {
	switch text {
	case "+":  return .Add, true
	case "-":  return .Sub, true
	case "*":  return .Mul, true
	case "/":  return .Div, true
	case "%":  return .Mod, true
	case "&":  return .Bit_And, true
	case "|":  return .Bit_Or, true
	case "~":  return .Bit_Xor, true
	case "<<": return .Shift_Left, true
	case ">>": return .Shift_Right, true
	case "&~": return .And_Not, true
	case "==": return .Equal, true
	case "!=": return .Not_Equal, true
	case "<":  return .Less, true
	case ">":  return .Greater, true
	case "<=": return .Less_Equal, true
	case ">=": return .Greater_Equal, true
	case "&&": return .Logical_And, true
	case "||": return .Logical_Or, true
	}
	return .Invalid, false
}

binary_result_type :: proc(op: Binary_Op, a, b: MIR_Type) -> MIR_Type {
	switch op {
	case .Equal, .Not_Equal, .Less, .Greater, .Less_Equal, .Greater_Equal, .Logical_And, .Logical_Or:
		return .Bool
	case:
		if a != .Invalid && a != .Bool do return a
		return b
	}
}

unary_kind :: proc(text: string) -> (Unary_Op, bool) {
	switch text {
	case "+": return .Positive, true
	case "-": return .Negative, true
	case "!": return .Logical_Not, true
	case "~": return .Bit_Not, true
	}
	return .Invalid, false
}

lower_logical_and :: proc(l: ^Lowerer, n: ^ast.Binary_Expr) -> (Value_ID, bool) {
	result := new_temp(l, .Bool)
	false_value := new_literal(l, "false", .Bool)
	emit_op(l, MIR_Inst{kind = .Assign, dst = result, a = false_value, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})

	left, ok := lower_expr(l, n.left)
	if !ok do return INVALID_VALUE, false
	rhs_block := new_block(l)
	end_block := new_block(l)
	emit_jump_if_false(l, left, end_block)

	begin_block(l, rhs_block)
	right, ok := lower_expr(l, n.right)
	if !ok do return INVALID_VALUE, false
	emit_op(l, MIR_Inst{kind = .Assign, dst = result, a = right, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})
	emit_jump(l, end_block)

	begin_block(l, end_block)
	return result, true
}

lower_logical_or :: proc(l: ^Lowerer, n: ^ast.Binary_Expr) -> (Value_ID, bool) {
	result := new_temp(l, .Bool)
	left, ok := lower_expr(l, n.left)
	if !ok do return INVALID_VALUE, false

	true_block := new_block(l)
	rhs_block := new_block(l)
	end_block := new_block(l)
	emit_jump_if_false(l, left, rhs_block)

	begin_block(l, true_block)
	true_value := new_literal(l, "true", .Bool)
	emit_op(l, MIR_Inst{kind = .Assign, dst = result, a = true_value, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})
	emit_jump(l, end_block)

	begin_block(l, rhs_block)
	right, ok := lower_expr(l, n.right)
	if !ok do return INVALID_VALUE, false
	emit_op(l, MIR_Inst{kind = .Assign, dst = result, a = right, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})
	emit_jump(l, end_block)

	begin_block(l, end_block)
	return result, true
}

lower_expr :: proc(l: ^Lowerer, expr: ^ast.Expr) -> (Value_ID, bool) {
	if expr == nil {
		lower_fail(l, nil, "nil expression")
		return INVALID_VALUE, false
	}

	#partial switch n in expr.derived {
	case ^ast.Ident:
		if n.name == "nil" {
			return new_value(l, .Null, .U8_Ptr, literal = "NULL"), true
		}
		if id, found := lookup_value(l, n.name); found do return id, true
		lower_fail(l, &expr.node, "unknown identifier %s", n.name)
		return INVALID_VALUE, false

	case ^ast.Basic_Lit:
		t := MIR_Type.UIntptr
		if n.tok.kind == tokenizer.Token_Kind.Rune do t = .U8
		return new_literal(l, n.tok.text, t), true

	case ^ast.Paren_Expr:
		return lower_expr(l, n.expr)

	case ^ast.Index_Expr:
		base, ok := lower_expr(l, n.expr)
		if !ok do return INVALID_VALUE, false
		index, ok := lower_expr(l, n.index)
		if !ok do return INVALID_VALUE, false
		result_type := MIR_Type.Invalid
		if value_type(l, base) == .U8_Ptr do result_type = .U8
		if result_type == .Invalid {
			lower_fail(l, &expr.node, "unsupported index base type")
			return INVALID_VALUE, false
		}
		dst := new_temp(l, result_type)
		emit_op(l, MIR_Inst{kind = .Load_Index, type = result_type, dst = dst, a = base, b = index, callee = INVALID_PROC, target = INVALID_BLOCK})
		return dst, true

	case ^ast.Call_Expr:
		if name, named := mir_ident_name(n.expr); named {
			if cast_type := mir_type_from_ast(n.expr); cast_type != .Invalid {
				if len(n.args) != 1 {
					lower_fail(l, &expr.node, "scalar cast argument count")
					return INVALID_VALUE, false
				}
				source, ok := lower_expr(l, n.args[0])
				if !ok do return INVALID_VALUE, false
				dst := new_temp(l, cast_type)
				emit_op(l, MIR_Inst{kind = .Cast, type = cast_type, dst = dst, a = source, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})
				return dst, true
			}
			proc_id, found := l.proc_ids[name]
			if !found {
				lower_fail(l, &expr.node, "unknown procedure %s", name)
				return INVALID_VALUE, false
			}
			first := u32(len(l.m.call_args))
			for arg in n.args {
				id, ok := lower_expr(l, arg)
				if !ok do return INVALID_VALUE, false
				append(&l.m.call_args, id)
			}
			result_type := l.m.procedures[int(proc_id)].result
			dst := INVALID_VALUE
			if result_type != .Void do dst = new_temp(l, result_type)
			emit_op(l, MIR_Inst{kind = .Call, type = result_type, dst = dst, a = INVALID_VALUE, b = INVALID_VALUE, callee = proc_id, args_first = first, args_count = u32(len(n.args)), target = INVALID_BLOCK})
			return dst, true
		}
		lower_fail(l, &expr.node, "indirect calls are not in the bootstrap MIR")
		return INVALID_VALUE, false

	case ^ast.Unary_Expr:
		op, supported := unary_kind(n.op.text)
		if !supported {
			lower_fail(l, &expr.node, "unary operator %s", n.op.text)
			return INVALID_VALUE, false
		}
		a, ok := lower_expr(l, n.expr)
		if !ok do return INVALID_VALUE, false
		result_type := value_type(l, a)
		if op == .Logical_Not do result_type = .Bool
		dst := new_temp(l, result_type)
		emit_op(l, MIR_Inst{kind = .Unary, type = result_type, dst = dst, a = a, b = INVALID_VALUE, unary_op = op, callee = INVALID_PROC, target = INVALID_BLOCK})
		return dst, true

	case ^ast.Binary_Expr:
		if n.op.text == "&&" do return lower_logical_and(l, n)
		if n.op.text == "||" do return lower_logical_or(l, n)
		if n.op.text == "..<" || n.op.text == "..=" {
			lower_fail(l, &expr.node, "range expression outside range statement")
			return INVALID_VALUE, false
		}
		op, supported := binary_kind(n.op.text)
		if !supported {
			lower_fail(l, &expr.node, "binary operator %s", n.op.text)
			return INVALID_VALUE, false
		}
		a, ok := lower_expr(l, n.left)
		if !ok do return INVALID_VALUE, false
		b, ok := lower_expr(l, n.right)
		if !ok do return INVALID_VALUE, false
		result_type := binary_result_type(op, value_type(l, a), value_type(l, b))
		dst := new_temp(l, result_type)
		emit_op(l, MIR_Inst{kind = .Binary, type = result_type, dst = dst, a = a, b = b, bin_op = op, callee = INVALID_PROC, target = INVALID_BLOCK})
		return dst, true

	case ^ast.Type_Cast:
		t := mir_type_from_ast(n.type)
		if t == .Invalid {
			lower_fail(l, &expr.node, "cast type")
			return INVALID_VALUE, false
		}
		a, ok := lower_expr(l, n.expr)
		if !ok do return INVALID_VALUE, false
		dst := new_temp(l, t)
		emit_op(l, MIR_Inst{kind = .Cast, type = t, dst = dst, a = a, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})
		return dst, true
	}

	lower_fail(l, &expr.node, "expression AST kind")
	return INVALID_VALUE, false
}

assignment_binary :: proc(text: string) -> (Binary_Op, bool) {
	switch text {
	case "+=":  return .Add, true
	case "-=":  return .Sub, true
	case "*=":  return .Mul, true
	case "/=":  return .Div, true
	case "%=":  return .Mod, true
	case "&=":  return .Bit_And, true
	case "|=":  return .Bit_Or, true
	case "~=":  return .Bit_Xor, true
	case "<<=": return .Shift_Left, true
	case ">>=": return .Shift_Right, true
	case "&~=": return .And_Not, true
	}
	return .Invalid, false
}

lower_assign :: proc(l: ^Lowerer, n: ^ast.Assign_Stmt) -> bool {
	if len(n.lhs) != 1 || len(n.rhs) != 1 do return lower_fail(l, &n.node, "tuple assignment")
	rhs, ok := lower_expr(l, n.rhs[0])
	if !ok do return false

	if name, named := mir_ident_name(n.lhs[0]); named {
		dst, found := lookup_value(l, name)
		if !found do return lower_fail(l, &n.node, "assignment to unknown %s", name)
		if n.op.text == "=" {
			emit_op(l, MIR_Inst{kind = .Assign, type = value_type(l, dst), dst = dst, a = rhs, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})
			return true
		}
		op, supported := assignment_binary(n.op.text)
		if !supported do return lower_fail(l, &n.node, "assignment operator %s", n.op.text)
		tmp := new_temp(l, binary_result_type(op, value_type(l, dst), value_type(l, rhs)))
		emit_op(l, MIR_Inst{kind = .Binary, type = value_type(l, tmp), dst = tmp, a = dst, b = rhs, bin_op = op, callee = INVALID_PROC, target = INVALID_BLOCK})
		emit_op(l, MIR_Inst{kind = .Assign, type = value_type(l, dst), dst = dst, a = tmp, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})
		return true
	}

	if index, is_index := n.lhs[0].derived.(^ast.Index_Expr); is_index {
		if n.op.text != "=" do return lower_fail(l, &n.node, "compound indexed assignment")
		base, ok := lower_expr(l, index.expr)
		if !ok do return false
		idx, ok := lower_expr(l, index.index)
		if !ok do return false
		emit_op(l, MIR_Inst{kind = .Store_Index, type = value_type(l, rhs), dst = base, a = idx, b = rhs, callee = INVALID_PROC, target = INVALID_BLOCK})
		return true
	}

	return lower_fail(l, &n.node, "assignment lvalue")
}

lower_local :: proc(l: ^Lowerer, n: ^ast.Value_Decl) -> bool {
	if len(n.names) != 1 || len(n.values) != 1 do return lower_fail(l, &n.node, "multi-name local declaration")
	name, named := mir_ident_name(n.names[0])
	if !named do return lower_fail(l, &n.node, "local name")

	init, ok := lower_expr(l, n.values[0])
	if !ok do return false
	t := mir_type_from_ast(n.type)
	if t == .Invalid do t = value_type(l, init)
	if t == .Invalid do return lower_fail(l, &n.node, "cannot infer local %s", name)

	dst := new_value(l, .Local, t, name = name)
	l.locals[name] = dst
	emit_op(l, MIR_Inst{kind = .Assign, type = t, dst = dst, a = init, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})
	return true
}

lower_if :: proc(l: ^Lowerer, n: ^ast.If_Stmt) -> bool {
	if n.init != nil do return lower_fail(l, &n.node, "if initializer")
	cond, ok := lower_expr(l, n.cond)
	if !ok do return false

	then_block := new_block(l)
	end_block := new_block(l)
	else_block := end_block
	if n.else_stmt != nil do else_block = new_block(l)

	emit_jump_if_false(l, cond, else_block)
	begin_block(l, then_block)
	if !lower_stmt(l, n.body) do return false
	if !block_terminated(l) do emit_jump(l, end_block)

	if n.else_stmt != nil {
		begin_block(l, else_block)
		if !lower_stmt(l, n.else_stmt) do return false
		if !block_terminated(l) do emit_jump(l, end_block)
	}

	begin_block(l, end_block)
	return true
}

lower_range :: proc(l: ^Lowerer, n: ^ast.Range_Stmt) -> bool {
	if n.reverse || len(n.vals) != 1 do return lower_fail(l, &n.node, "reverse/multi-value range")
	name, named := mir_ident_name(n.vals[0])
	if !named do return lower_fail(l, &n.node, "range index")
	range_expr, is_range := n.expr.derived.(^ast.Binary_Expr)
	if !is_range || (range_expr.op.text != "..<" && range_expr.op.text != "..=") {
		return lower_fail(l, &n.node, "non-bounded range")
	}

	first, ok := lower_expr(l, range_expr.left)
	if !ok do return false
	limit, ok := lower_expr(l, range_expr.right)
	if !ok do return false
	idx_type := value_type(l, limit)
	if idx_type == .Invalid do idx_type = .UIntptr
	index := new_value(l, .Local, idx_type, name = name)
	l.locals[name] = index
	emit_op(l, MIR_Inst{kind = .Assign, type = idx_type, dst = index, a = first, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})

	cond_block := new_block(l)
	body_block := new_block(l)
	end_block := new_block(l)
	emit_jump(l, cond_block)

	begin_block(l, cond_block)
	cmp_op := Binary_Op.Less
	if range_expr.op.text == "..=" do cmp_op = .Less_Equal
	cond := new_temp(l, .Bool)
	emit_op(l, MIR_Inst{kind = .Binary, type = .Bool, dst = cond, a = index, b = limit, bin_op = cmp_op, callee = INVALID_PROC, target = INVALID_BLOCK})
	emit_jump_if_false(l, cond, end_block)

	begin_block(l, body_block)
	if !lower_stmt(l, n.body) do return false
	if !block_terminated(l) {
		one := new_literal(l, "1", idx_type)
		next := new_temp(l, idx_type)
		emit_op(l, MIR_Inst{kind = .Binary, type = idx_type, dst = next, a = index, b = one, bin_op = .Add, callee = INVALID_PROC, target = INVALID_BLOCK})
		emit_op(l, MIR_Inst{kind = .Assign, type = idx_type, dst = index, a = next, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})
		emit_jump(l, cond_block)
	}

	begin_block(l, end_block)
	return true
}

lower_stmt :: proc(l: ^Lowerer, stmt: ^ast.Stmt) -> bool {
	if stmt == nil do return true
	if block_terminated(l) do return true

	#partial switch n in stmt.derived {
	case ^ast.Block_Stmt:
		for child in n.stmts {
			if block_terminated(l) do break
			if !lower_stmt(l, child) do return false
		}
		return true
	case ^ast.Value_Decl:
		return lower_local(l, n)
	case ^ast.Assign_Stmt:
		return lower_assign(l, n)
	case ^ast.Expr_Stmt:
		_, ok := lower_expr(l, n.expr)
		return ok
	case ^ast.Return_Stmt:
		if len(n.results) > 1 do return lower_fail(l, &n.node, "multiple return values")
		value := INVALID_VALUE
		if len(n.results) == 1 {
			var ok: bool
			value, ok = lower_expr(l, n.results[0])
			if !ok do return false
		}
		emit_op(l, MIR_Inst{kind = .Return, a = value, dst = INVALID_VALUE, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})
		l.m.blocks[int(l.current_block)].terminated = true
		return true
	case ^ast.If_Stmt:
		return lower_if(l, n)
	case ^ast.Range_Stmt:
		return lower_range(l, n)
	}

	return lower_fail(l, &stmt.node, "statement AST kind")
}

const_init_text :: proc(expr: ^ast.Expr) -> (string, MIR_Type, bool) {
	if expr == nil do return "", .Invalid, false
	#partial switch n in expr.derived {
	case ^ast.Basic_Lit:
		t := MIR_Type.UIntptr
		if n.tok.kind == tokenizer.Token_Kind.Rune do t = .U8
		return n.tok.text, t, true
	case ^ast.Paren_Expr:
		return const_init_text(n.expr)
	case ^ast.Call_Expr:
		if len(n.args) == 1 {
			if name, named := mir_ident_name(n.expr); named {
				t := mir_type_from_ast(n.expr)
				if t != .Invalid {
					text, _, ok := const_init_text(n.args[0])
					return text, t, ok
				}
			}
		}
	}
	return "", .Invalid, false
}

sorted_files :: proc(pkg: ^ast.Package) -> [dynamic]string {
	paths := make([dynamic]string, 0, len(pkg.files), context.temp_allocator)
	for path, _ in pkg.files do append(&paths, path)
	slice.sort(paths[:])
	return paths
}

collect_module_symbols :: proc(l: ^Lowerer, pkg: ^ast.Package) -> bool {
	paths := sorted_files(pkg)
	for path in paths {
		for stmt in pkg.files[path].decls {
			d, is_decl := stmt.derived.(^ast.Value_Decl)
			if !is_decl || len(d.names) != 1 || len(d.values) != 1 do continue
			name, named := mir_ident_name(d.names[0])
			if !named do return lower_fail(l, &stmt.node, "top-level declaration name")

			if p, is_proc := d.values[0].derived.(^ast.Proc_Lit); is_proc {
				result := mir_proc_result(p)
				if result == .Invalid do return lower_fail(l, &stmt.node, "procedure result type for %s", name)
				id := Proc_ID(len(l.m.procedures))
				append(&l.m.procedures, MIR_Procedure{name = name, result = result, calling_convention = mir_calling_convention(p), external = mir_proc_external(p), entry = INVALID_BLOCK})
				l.proc_ids[name] = id
				continue
			}

			init, init_type, ok := const_init_text(d.values[0])
			if !ok do return lower_fail(l, &stmt.node, "top-level constant initializer for %s", name)
			t := mir_type_from_ast(d.type)
			if t == .Invalid do t = init_type
			if t == .Invalid do return lower_fail(l, &stmt.node, "top-level constant type for %s", name)
			value_id := new_value(l, .Global, t, name = name)
			append(&l.m.globals, MIR_Global{name = name, type = t, init = init, value_id = value_id})
			l.global_ids[name] = value_id
		}
	}
	return true
}

lower_procedure :: proc(l: ^Lowerer, id: Proc_ID, p: ^ast.Proc_Lit) -> bool {
	proc := &l.m.procedures[int(id)]
	l.current_proc = id
	l.current_block = INVALID_BLOCK
	delete(l.locals)
	l.locals = make(map[string]Value_ID)

	proc.first_value = u32(len(l.m.values))
	proc.param_first = proc.first_value
	if p.type.params != nil {
		for f in p.type.params.list {
			t := mir_type_from_ast(f.type)
			if t == .Invalid do return lower_fail(l, &p.node, "procedure parameter type in %s", proc.name)
			for name_expr in f.names {
				name, named := mir_ident_name(name_expr)
				if !named do return lower_fail(l, &p.node, "procedure parameter name in %s", proc.name)
				value_id := new_value(l, .Param, t, name = name)
				l.locals[name] = value_id
				proc.param_count += 1
			}
		}
	}

	proc.first_block = u32(len(l.m.blocks))
	proc.first_op = u32(len(l.m.ops))
	entry := new_block(l)
	proc.entry = entry
	begin_block(l, entry)
	if !lower_stmt(l, p.body) do return false
	if !block_terminated(l) {
		if proc.result == .Void {
			emit_op(l, MIR_Inst{kind = .Return, a = INVALID_VALUE, dst = INVALID_VALUE, b = INVALID_VALUE, callee = INVALID_PROC, target = INVALID_BLOCK})
			l.m.blocks[int(l.current_block)].terminated = true
		} else {
			return lower_fail(l, &p.node, "non-void procedure %s can fall through", proc.name)
		}
	}

	proc.block_count = u32(len(l.m.blocks)) - proc.first_block
	proc.op_count = u32(len(l.m.ops)) - proc.first_op
	proc.value_count = u32(len(l.m.values)) - proc.first_value
	return true
}

lower_package_to_mir :: proc(pkg: ^ast.Package) -> (m: MIR_Module, ok: bool) {
	l := Lowerer{
		m = &m,
		proc_ids = make(map[string]Proc_ID),
		global_ids = make(map[string]Value_ID),
		locals = make(map[string]Value_ID),
		current_proc = INVALID_PROC,
		current_block = INVALID_BLOCK,
	}
	defer delete(l.proc_ids)
	defer delete(l.global_ids)
	defer delete(l.locals)

	if !collect_module_symbols(&l, pkg) do return m, false

	paths := sorted_files(pkg)
	for path in paths {
		for stmt in pkg.files[path].decls {
			d, is_decl := stmt.derived.(^ast.Value_Decl)
			if !is_decl || len(d.names) != 1 || len(d.values) != 1 do continue
			p, is_proc := d.values[0].derived.(^ast.Proc_Lit)
			if !is_proc do continue
			name, named := mir_ident_name(d.names[0])
			if !named do return m, false
			id := l.proc_ids[name]
			if !lower_procedure(&l, id, p) do return m, false
		}
	}

	return m, !l.failed
}
