...
infer_type :: proc(e: ^Emitter, expr: ^ast.Expr) -> C_Type {
	...
	#partial switch n in expr.derived {
	case ^ast.Ident:
		if n.name == "true" || n.name == "false" do return .Bool
	...
	case ^ast.Binary_Expr:
		...
		if _, literal := n.left.derived.(^ast.Basic_Lit); literal && right_type != .Unknown do left_type = right_type
...
emit_expr :: proc(e: ^Emitter, expr: ^ast.Expr) -> bool {
	...
	case ^ast.Basic_Lit:
		if n.tok.kind == .Integer || n.tok.kind == .Rune {
			if _, valid := mir_literal_number(n.tok.text); !valid do return fail(e, "integer/rune literal out of range")
			write_literal_c(&e.out, n.tok.text)
		} else {
			write(e, n.tok.text)
		}
		return true
	...
	case ^ast.Binary_Expr:
		if n.op.text == "..<" || n.op.text == "..=" do return fail(e, "range expression outside range statement")
		t := infer_type(e, expr)
		ct, known := c_type_name(t)
		numeric := known && t != .Bool && t != .U8_Ptr && t != .Void
		if numeric && (n.op.text == "<<" || n.op.text == ">>") {
			e.shift_helpers[fmt.tprintf("%s_%s", n.op.text == "<<" ? "shl" : "shr", ct)] = true
			write(e, n.op.text == "<<" ? "bor_c99_shl_" : "bor_c99_shr_")
			write(e, ct)
			write(e, "(")
		...
		if numeric { write(e, "(("); write(e, ct); write(e, ")(") } else { write(e, "(") }
		if !emit_direct_operand(e, n.left, t, numeric) do return false
		write(e, n.op.text == "&~" ? " & ~(" : n.op.text == "~" ? " ^ " : fmt.tprintf(" %s ", n.op.text))
		if !emit_direct_operand(e, n.right, t, numeric) do return false
		if n.op.text == "&~" do write(e, ")")
		write(e, numeric ? "))" : ")")
		return true
...
emit_c99 :: proc(pkg: ^ast.Package) -> (string, bool) {
	...
	// Emitting bodies discovers exactly the helpers used by this translation
	// unit. Prefix only those helpers, keeping strict unused-function checks.
	body_builder := e.out
	e.out = {}
	strings.builder_init(&e.out)
	write(&e, "/* generated by Bor from core:odin/parser AST */\n")
	write(&e, "#include <limits.h>\n#include <stdbool.h>\n#include <stddef.h>\n#include <stdint.h>\n\n")
	emit_direct_shift_helpers(&e)
	write(&e, strings.to_string(body_builder))
	strings.builder_destroy(&body_builder)
