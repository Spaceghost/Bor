...
proc_is_c :: proc(p: ^ast.Proc_Lit) -> bool {
	...
	if cc, ok := p.type.calling_convention.(string); ok {
		return cc == "c"
	}
	return false
}
...
infer_type :: proc(e: ^Emitter, expr: ^ast.Expr) -> C_Type {
	...
	case ^ast.Ident:
		...
		if t, ok := e.proc_results[n.name]; ok {
			return t
		}
		return builtin_type(n.name)
	case ^ast.Basic_Lit:
	...
	case ^ast.Call_Expr:
		if name, ok := ident_name(n.expr); ok {
			if t := builtin_type(name); t != .Unknown {
				return t
			}
			if t, found := e.proc_results[name]; found {
				return t
			}
		}
	case ^ast.Unary_Expr:
	...
	case ^ast.Binary_Expr:
		...
		lt := infer_type(e, n.left)
		rt := infer_type(e, n.right)
		if lt != .Unknown && lt != .Bool {
			return lt
		}
		return rt
	}
...
emit_block :: proc(e: ^Emitter, stmt: ^ast.Stmt) -> bool {
	...
	b, ok := stmt.derived.(^ast.Block_Stmt)
	if !ok {
...
emit_stmt :: proc(e: ^Emitter, stmt: ^ast.Stmt) -> bool {
	...
	case ^ast.If_Stmt:
		...
		if n.else_stmt != nil {
			indent(e)
			write(e, "else ")
			if _, ok := n.else_stmt.derived.(^ast.If_Stmt); ok {
	...
	case ^ast.Range_Stmt:
		...
		idx, ok := ident_name(n.vals[0])
		if !ok {
			return fail(e, "range index")
		}
		r, ok := n.expr.derived.(^ast.Binary_Expr)
		if !ok || (r.op.text != "..<" && r.op.text != "..=") {
			return fail(e, "non-bounded range")
		}
		t := infer_type(e, r.right)
		if t == .Unknown {
			t = .UIntptr
		}
		ct, ok := c_type_name(t)
		...
		write(e, idx)
		write(e, " = ")
		if !emit_expr(e, r.left) do return false
		write(e, "; ")
		write(e, idx)
		write(e, r.op.text == "..<" ? " < " : " <= ")
		if !emit_expr(e, r.right) do return false
		write(e, "; ++")
		write(e, idx)
		write(e, ") ")
...
emit_proc_head :: proc(e: ^Emitter, name: string, p: ^ast.Proc_Lit, prototype: bool) -> bool {
	rt := proc_result_type(p)
	ct, ok := c_type_name(rt)
	if !ok {
...
collect_globals :: proc(e: ^Emitter, pkg: ^ast.Package) -> bool {
	for _, file in pkg.files {
		for stmt in file.decls {
			d, ok := stmt.derived.(^ast.Value_Decl)
			if !ok || len(d.names) != 1 || len(d.values) != 1 {
				continue
			}
			name, ok := ident_name(d.names[0])
			if !ok do continue
			if p, ok := d.values[0].derived.(^ast.Proc_Lit); ok {
...
main :: proc() {
	...
	pkg, ok := parser.parse_package_from_path(os.args[2])
	if !ok || pkg == nil {
