...
uint32_t bor_test_unions(void);
uint32_t bor_test_imports(void);
uint32_t bor_test_generic(void);
...
int main(void) {
    ...
    assert(bor_test_unions() == 49);
    assert(bor_test_imports() == 73);
    assert(bor_test_generic() == 77);
    return 0;
}
